...
class Pak:
    ...
    def read_pak(self, path):
        ...
        self.data = data
        self.align = struct.unpack('I', data[0x0:0x4])[0]
    ...
    def repack(self, indir, outpath):
        ...
        with open(outpath, 'wb') as fp:
            fp.write(struct.pack('<II', self.align, self.itemnum))
            fp.write(self.data[0x8:0x14])
            for i in range(self.itemnum):
...
def debug():
    #extract_pak(r"H:\MAKE\GalagmeReverse\tmp\FONT.PAK", r"H:\MAKE\GalagmeReverse\tmp\font")
    insert_pak(r"H:\MAKE\GalagmeReverse\tmp\FONT.PAK", r"H:\MAKE\GalagmeReverse\tmp\FONT", r"H:\MAKE\GalagmeReverse\tmp\rebuild\FONT.PAK")
