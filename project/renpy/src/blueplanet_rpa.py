"""
extract *.dll file for Dream of BluePlanet
...
def extract_rpa(inpath, outdir="out"):
    with open(inpath, "rb") as fp:
        data = fp.read()
        index = RPAv3ArchiveHandler.read_index(io.BytesIO(data))
    
    fp = open(inpath.replace(".rpa", ".zip"), "rb")
...
def cli(argv=sys.argv):
    if len(argv) < 2:
        print("blueplannet_rpa inpath [outdir] // inpath *.dll")
