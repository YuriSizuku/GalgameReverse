...
advhdentry_t = namedtuple('advhdentry_t', ['size', 'offset', 'name'])
...
def parse_arc(data: bytes) -> Tuple[int, List[advhdentry_t]]:
    n, index_size = struct.unpack("<II", data[0:8])
    cur = 8
    adventries: List[advhdentry_t] = []
    while cur < index_size + 8:
        ...
        cur = end + 2
        adventries.append(advhdentry_t(size, offset, name))
...
def build_arc(indir, outpath="./out.bin"):
    # prepare index
    offset = 0
    index_size = 0
    entries: List[advhdentry_t] = []
    names = os.listdir(indir)
    for name in names:
        inpath = os.path.join(indir, name)
        size = os.stat(inpath).st_size
        entries.append(advhdentry_t(size, offset, name))
        offset += size
...
def debug():
    # extract_arc("./build/intermediate/origin/Rio2.arc")
    build_arc("./build/intermediate/Rio2")
    pass
